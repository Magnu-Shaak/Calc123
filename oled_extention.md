# OLED extension
This is my OLED extension I made in CircuitPy that is compatable to use with KMK. 
I Don't believe anyone will see this, but if you are, use the oled_layer_extension.py file, under the Firmware folder. The oled_extension is configured to the board I am using, and my own oled modifications


## List of Supported features below:

Event Updating:
- Layer Changes
    
    
Display Chips:
- SSD 1306

## Configure
```python
# main.py or code.py
from oled_extension import init_oled                # Import

# Name this whatever, just be consistent with the name when you call the  init_oled function
layer_names_map = {  
    0: "Numpad",       # Map the text displayed to layer index
    1: "Functions",    # If no text is defined, it defaults to "Layer x"
}

# Input your entries for each of the feilds. If no entry is added, some feilds have default values set
init_oled(keyboard, layer_names_map, scl_pin, sda_pin, display_hight=32, display_width=128, display_address=0x3C)

# For Example:
init_oled(keyboard, layer_names_map, board.D5, board.D4, 64, 128, 0x3C)
```
## Other
Comment on this repo if you want me to add anythin. Image support and Mod Layers as events will be added promptly upon request.
